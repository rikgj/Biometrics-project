...
def main():
    ...
    # make a filter list based on images in save_dir
    filter_names = []
    EXTENSION = '*.jpg'
    search = save_dir + '/' + EXTENSION
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        if imgname in filter_names:
            # skip image if present in save_dir
            print(imgname)
            continue
...
def setup(args):
    ...
    # 003 real-world image sr
        ...
        prefix = f'{DIM_FOLDER}_{int(DIM_FOLDER/int(args.scale))}'
        save_dir = f'../images/{DIM_FOLDER}/{prefix}swinir_x{args.scale}'
        if args.large_model:
            save_dir += '_large'
